...
An API key is a secret token that lets złotówa send requests to an LLM service on your behalf. The provider uses it to identify your account and bill you for usage. Think of it as a password for programmatic access.
...
złotówa uses small, cheap models. Classifying expenses costs fractions of a cent per batch, so even $5 in API credits will last a long time.
...
4. In złotówa, select **Ollama** and leave the endpoint as the default (or enter a custom URL if you changed it)

## Setup (GUI)

1. Open złotówa and go to **Settings** in the sidebar
