<script>
  let { pendingRules, onsave, onskip } = $props();
...
<div class="max-w-3xl">
  ...
  {#if rules.length === 0}
    <div class="bg-gray-900 rounded-xl border border-gray-800 p-8 text-center mb-4">
      ...
      <p class="text-sm text-gray-500 mb-4">All expenses were matched by existing rules and no categories were changed.</p>
      <button
        onclick={onskip}
        class="px-5 py-2.5 rounded-lg bg-amber-500 hover:bg-amber-400 text-gray-950 text-sm font-medium transition-colors"
      >
        Continue
      </button>
    </div>
  {:else if activeRules.length === 0 && removedCount > 0}
    <div class="bg-gray-900 rounded-xl border border-gray-800 p-8 text-center mb-4">
      <p class="text-gray-400 mb-3">All rules have been removed.</p>
      <button
        onclick={onskip}
  ...
  <div class="flex gap-3 mt-6">
    {#if activeRules.length > 0}
      <button
        onclick={handleSave}
        class="px-5 py-2.5 rounded-lg bg-amber-500 hover:bg-amber-400 text-gray-950
    ...
    <button
      onclick={onskip}
      class="px-5 py-2.5 rounded-lg bg-gray-800 hover:bg-gray-700 text-gray-300
